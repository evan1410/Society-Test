...
namespace webPortal
{
    public partial class cashvoucher : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static string LoadCashVoucher(string cvid)
        {
            DataTable dtListing;
            clsTransaction objTrn = new clsTransaction();
            StringBuilder strbld = new StringBuilder();
            ...
            string[] strSelect = new string[] { cvid.ToString() };
            dtListing = objTrn.Transaction(strSelect, clsConnection.GetEnumDescription(clsEnumSP.usp_Select_CashVoucher));
            ...
            if (cvid == "0")
            {
                if (dtListing != null)
                {
                    ...
                    strbld.Append("<thead>");
                    strbld.Append("<tr>");
                    strbld.Append("<th class=\"hide\">cvid</th>");
                    ...
                    {
                        strbld.Append("<tr>");
                        strbld.Append("<td class=\"hide\">" + dr["cvid"].ToString() + "</td>");
                        ...
                        strbld.Append("<td class=\"text-center\"><i class=\"fa fa-eye\" aria-hidden=\"true\" title=\"View\" onclick=\"LoadCashVoucher(" + dr["cvid"].ToString() + ")\">&nbsp&nbsp</i><i class=\"fa fa-trash\" title=\"Delete\" aria-hidden=\"true\" onclick=\"DeleteRecord(" + dr["cvid"].ToString() + ")\"></i></td>");
                        strbld.Append("</tr>");
                    }
                    strbld.Append("</tbody>");
                    //strbld.Append("</table>");
            ...
            else
            {
                if (dtListing != null)
                {
                    foreach (DataRow dr in dtListing.Rows)
                    {
                        strbld.Append(dr["cvid"].ToString() + "|");
        ...
        [WebMethod]
        public static string SaveCashVoucher(string cvid, string voucherno, string voucherdate, string ledgerfrom, string trandesc, string subtotal,
                                         string discount, string gst, string totalamt)
        {
            DataTable dtCashVoucher = new DataTable();
            clsTransaction objTrn = new clsTransaction();
            StringBuilder strbld = new StringBuilder();
            ...
            if (cvid == "0")
            {
                string[] strInsert = new string[] {cvid.ToString()
                                                    ,voucherno.ToString()
                ...
                dtCashVoucher = objTrn.Transaction(strInsert, clsConnection.GetEnumDescription(clsEnumSP.usp_DML_CashVoucher));

                strBld = dtCashVoucher.Rows[0]["i_IDENTITY"].ToString() + '|' + dtCashVoucher.Rows[0]["msg"].ToString();
            }
            else
            {
                string[] strUpdate = new string[] { cvid.ToString()
                                                    ,voucherno.ToString()
                                                    ,Convert.ToDateTime(voucherdate).ToString("yyyy-MM-dd")
                                                    ,ledgerfrom.ToString()
                                                    ,trandesc.ToString()
                ...
                dtCashVoucher = objTrn.Transaction(strUpdate, clsConnection.GetEnumDescription(clsEnumSP.usp_DML_CashVoucher));
        ...
        [WebMethod]
        public static string SaveCashDtls(string id, string cvid, string ledgerto, string qty, string rate, string amount, string gstamt, string total)
        {
            DataTable dtCashVoucher = new DataTable();
            clsTransaction objTrn = new clsTransaction();
            StringBuilder strbld = new StringBuilder();
            ...
            if (cvid == "0")
            {
                ...
                dtCashVoucher = objTrn.Transaction(strInsert, clsConnection.GetEnumDescription(clsEnumSP.usp_DML_CashVoucherDtls));
            ...
            else
            {
                ...
                dtCashVoucher = objTrn.Transaction(strUpdate, clsConnection.GetEnumDescription(clsEnumSP.usp_DML_CashVoucherDtls));
        ...
        [WebMethod]
        public static string DeleteCash(string cvid)
        {
            DataTable dtCashVoucher = new DataTable();
            clsTransaction objTrn = new clsTransaction();
            StringBuilder strbld = new StringBuilder();
            ...
            string[] strDelete = new string[] { cvid.ToString()
                                                ,""
                                                ,""
                                                ,""
                                                ,""
            ...
            dtCashVoucher = objTrn.Transaction(strDelete, clsConnection.GetEnumDescription(clsEnumSP.usp_DML_CashVoucher));
            dtCashVoucher = objTrn.Transaction(strDelete, clsConnection.GetEnumDescription(clsEnumSP.usp_DML_CashVoucherDtls));
